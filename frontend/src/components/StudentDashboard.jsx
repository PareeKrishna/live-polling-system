...
const StudentDashboard = () => {
    ...
    useEffect(() => {
        ...
        // but clears when the tab is closed or a new tab is opened
    ...
    useEffect(() => {
        socket.on("poll-started", (q) => {
            ...
            setResult(null);
            setSelectedOption("");
            // Restore submitted state from sessionStorage so navigating away and back doesn't reset it
            const wasSubmitted = sessionStorage.getItem(`submitted_${q._id}`) === 'true';
            setSubmitted(wasSubmitted);
            // Use timeRemaining if available (e.g. after refresh), else fall back to timeLimit
            setTimer(q.timeRemaining ?? q.timeLimit ?? 60);
        });
    ...
    const handleSubmit = () => {
        ...
        setSubmitted(true);
        // Persist so navigating away and back doesn't re-show the timer
        sessionStorage.setItem(`submitted_${question._id}`, 'true');
    };
